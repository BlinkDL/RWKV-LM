...
datafile = "V:\\NLP\\simplebooks\\simplebooks-92-raw\\train.txt"
...
lr_init = (
    6e-4 if model_type == "RWKV" else 4e-4
...
eps = 4e-9
weight_decay = (
    0 if model_type == "RWKV" else 0.01
)  # wd is not useful when we have enough data

epoch_length_fixed = (
    10000  # make an 'epoch' very short, so we can see the training progress
)
...
train_dataset = Dataset(
    open(datafile, "r", encoding=datafile_encoding).read(), model_level, ctx_len
)
