...
EXPRESS_PILE_MODE = (
    False  # True: express mode for fine-tuning a pile model // False: usual training
)
...
if (
    int(os.environ["RWKV_NUM_GPUS"]) == 1
):  # Usually you don't need DeepSpeed for 1 GPU training.
    os.environ[
        "RWKV_DEEPSPEED"
    ] = "0"  # However, sometimes DeepSpeed saves VRAM even for 1 GPU training. So you shall try it.
...
LOAD_MODEL = (
    False  # shall we load the #EPOCH_BEGIN model and continue the training from it?
)
...
epoch_length_fixed = (
    10000 // batch_size
) * batch_size  # feel free to increase it if you have lots of GPU
...
if (
    LOAD_MODEL and EPOCH_BEGIN > 0
):  # we are not saving gradients, so let's have some warmup if we load a model
...
betas = (
    0.9,
    0.99,
)  # set betas = (0.9, 0.999) if your model has been trained for a while
eps = 1e-8
...
if datafile_encoding == "binidx":
    ...
    train_dataset = Dataset(
        np.load(datafile).astype("int"), ctx_len, epoch_length_fixed
    )
else:
    train_dataset = Dataset(
...
if __name__ == "__main__":
    ...
    if os.environ["RWKV_DEEPSPEED"] == "0":
        ...
        if os.environ["RWKV_FLOAT_MODE"] == "fp16":
            ...
            trainer = Trainer(
                strategy=DeepSpeedStrategy(config=DEEPSPEED_CFG),
                devices=NUM_GPUS,
                accelerator="gpu",
                precision=16,
            )
            ...
            DEEPSPEED_CFG["bf16"] = {"enabled": True}
            trainer = Trainer(
                strategy=DeepSpeedStrategy(config=DEEPSPEED_CFG),
                devices=NUM_GPUS,
                accelerator="gpu",
