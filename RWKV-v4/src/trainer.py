...
class Trainer(LightningLite):
    ...
    def run(self, m_cfg, train_dataset, test_dataset, config):
        ...
        self.steps = self.EPOCH_BEGIN * (
            len(self.train_dataset) // (config.batch_size // NUM_GPUS)
        )
                ...
                if is_train:  # backprop and update the parameters
                    ...
                    if self.tokens < config.warmup_tokens:
                        ...
                        if progress >= 1:
                            ...
                            lr_mult = math.exp(
                                math.log(lr_final_factor) * pow(progress, 1)
                            )
                    ...
                    pbar.set_description(
                        f"miniE {epoch+1+self.EPOCH_BEGIN} s {self.steps} prog {progress*100.0:.2f}% : ppl {math.exp(self.avg_loss):.6f} loss {self.avg_loss:.6f} lr {lr:e}"
                    )
        ...
        for epoch in range(99999999):
            ...
            if self.cuda_id == 0:
                log_file.write(
                    f"{epoch+1+self.EPOCH_BEGIN} {self.avg_loss:.6f} {math.exp(self.avg_loss):.4f} {self.lr:.8f} {datetime.datetime.now()} {epoch+1} \n"
                )
                log_file.flush()
