...
class RWKV_RNN(MyModule):
    def __init__(self, args):
        ...
        with torch.no_grad():
            ...
            if "pos_emb_x" in keys:
                w["pos_emb"] = (w["pos_emb_x"] + w["pos_emb_y"]).reshape(
                    args.ctx_len + 1, -1
                )[:-1, :]
            keys = list(w.keys())
    ...
    def FF(self, x, state, i: int, time_mix_k, time_mix_r, kw, vw, rw):
        if self.FLOAT_MODE == "bf16":
            xk = x * time_mix_k + state[5 * i + 0].type(torch.bfloat16) * (
                1 - time_mix_k
            )
            xr = x * time_mix_r + state[5 * i + 0].type(torch.bfloat16) * (
                1 - time_mix_r
            )
            state[5 * i + 0] = x.float()
        elif self.FLOAT_MODE == "fp16":
    ...
    def SA(
        ...
        if self.FLOAT_MODE == "bf16":
            xk = x * time_mix_k + state[5 * i + 1].type(torch.bfloat16) * (
                1 - time_mix_k
            )
            xv = x * time_mix_v + state[5 * i + 1].type(torch.bfloat16) * (
                1 - time_mix_v
            )
            xr = x * time_mix_r + state[5 * i + 1].type(torch.bfloat16) * (
                1 - time_mix_r
            )
            state[5 * i + 1] = x.float()
        elif self.FLOAT_MODE == "fp16":
