import torch, math, random, numpy as np, torch.nn.functional as F
...
# LOAD_NAME = '251024_rosaQKV_L2_digit20' # acc = 30583/31554=96.9% for 1-20 digits,
LOAD_NAME = '251024_rosaQKV_L4_digit40'  # acc = 31127/31554=98.6% for 1-20 digits, 460264/464866=99.0% for 1-40 digits

weights = torch.load(f'{LOAD_NAME}.pth', map_location=device, mmap=True, weights_only=True)

DIGIT_MAX = int(LOAD_NAME[LOAD_NAME.find('digit') + 5:])
V, C, T = 13, 128, 129
...
def samx_qkv_slow(qqq, kkk, vvv):  # slow, only for reference
	...
	y = [-1] * n
	s = 2 * n + 1
	t: list[Optional[dict[int, int]]] = [None] * s
	f: list[int] = [-1] * s
	m: list[int] = [0] * s
	r: list[int] = [-1] * s
	t[0] = dict()
...
if torch.cuda.is_available():
	load(name='wind_backstepping', sources=['wkv7_cuda.cu', 'wkv7_op.cpp'], is_python_module=False, verbose=False, extra_cuda_cflags=flags)


def wind_backstepping_cpu(w, q, k, v, z, b):
	batch, sequence, head, channel = w.shape
	state = torch.zeros(batch, head, channel, channel, dtype=w.dtype, device=w.device)
	y = torch.empty_like(v)
	for t in range(sequence):
		qt = q[:, t]  # [B, H, C]
		wt = torch.exp(-torch.exp(w[:, t]))
		kt = k[:, t]
		vt = v[:, t]  # scalar per channel
		zt = z[:, t]  # kernel里的 a
		bt = b[:, t]
		sa = (state * zt.unsqueeze(-2)).sum(dim=-1)  # [B, H, C]
		state = (state * wt.unsqueeze(-2) + sa.unsqueeze(-1) * bt.unsqueeze(-2) + kt.unsqueeze(-2) * vt.unsqueeze(-1))
		y[:, t] = (state * qt.unsqueeze(-2)).sum(dim=-1)
	return y
...
def run_cuda_rwkv7g(q, w, k, v, a, b, use_cuda: bool = False):
	batch, sequence, head_channel = q.shape
	q, w, k, v, a, b = [i.view(batch, sequence, head_channel // 16, 16) for i in [q, w, k, v, a, b]]
	if use_cuda:
		return WindBackstepping.apply(w, q, k, v, a, b).view(batch, sequence, head_channel)
	return wind_backstepping_cpu(w, q, k, v, a, b).view(batch, sequence, head_channel)


class RWKVTimeMixX070(MyModule):
	def __init__(self, args, layer_id):
		...
		with torch.no_grad():
			...
			for i in range(C):
				ddd[0, 0, i] = i / C
			self.x_r = nn.Parameter(torch.ones_like(ddd) - torch.pow(ddd, 0.2 * ratio_1_to_almost0))
			self.x_w = nn.Parameter(torch.ones_like(ddd) - torch.pow(ddd, 0.9 * ratio_1_to_almost0))
			self.x_k = nn.Parameter(torch.ones_like(ddd) - torch.pow(ddd, 0.7 * ratio_1_to_almost0))
			self.x_v = nn.Parameter(torch.ones_like(ddd) - torch.pow(ddd, 0.7 * ratio_1_to_almost0))
			self.x_a = nn.Parameter(torch.ones_like(ddd) - torch.pow(ddd, 0.9 * ratio_1_to_almost0))
			self.x_g = nn.Parameter(torch.ones_like(ddd) - torch.pow(ddd, 0.2 * ratio_1_to_almost0))

			def ortho_init(x: torch.Tensor, scale: float):
				with torch.no_grad():
					shape = x.shape
					if len(shape) == 2:
	...
	def forward(self, x, v_first):
		...
		a = torch.sigmoid(self.a0 + (xa @ self.a1) @ self.a2)  # variable 'a' is 'in-context learning rate'
...
class FFN(nn.Module):
	...
	def forward(self, x):
		xx = self.time_shift(x) - x  # essential for grok (require 2 FFN)
		x = x + xx * self.x_k
...
if '_L4' in LOAD_NAME:
	class MODEL(nn.Module):
		def __init__(s):
			...
			s.rwkv1 = RWKVTimeMixX070(args, 0)
			...
			s.rwkv2 = RWKVTimeMixX070(args, 1)
			...
			s.rwkv3 = RWKVTimeMixX070(args, 1)
			...
			s.rwkv4 = RWKVTimeMixX070(args, 1)
	...
	raise ValueError(f'unsupported LOAD_NAME: {LOAD_NAME}')
...
def get_randint(digits):
	...
	x = random.randint(lo, 10 ** digits - 1)
	return x


TOK = {**{str(i): i for i in range(10)}, '+': 10, '-': 11, '=': 12}
S = '0123456789+-='
src = list()
ngood = 0
nall = 0
...
for DIGIT in range(1, DIGIT_MAX + 1):
	...
	for ii in range(1, 2 * DIGIT):
		...
		b = get_randint(bb)
		op = random.choice(['+', '-'])
		raw = str(a) + op + str(b) + '=' + str(a + b if op == '+' else a - b)
		raw = raw + '=' * (129 - len(raw))
		raw = [TOK[c] for c in list(raw)]
		src.append(raw)
	src = torch.tensor(src, device=device, dtype=torch.long)
	dst = src[:, 1:]
	out = model(src[:, :-1]).argmax(-1)
	for n in range(src.shape[0]):
		xx = str().join(S[t] for t in src[n, :-1].tolist())
		p1 = xx.find('=')
		p2 = p1 + xx[p1 + 1:].find('=')
		expect = xx[p1 + 1:p2 + 1]
		yy = str().join(S[t] for t in out[n, p1:p2].tolist())
		ngood += (src[n, p1 + 1:p2 + 1] == out[n, p1:p2]).sum().item()
		nall += (p2 - p1)
		print('in', xx[:p2 + 1], 'model', yy, 'check', str().join(['.' if str(expect)[i] == str(yy)[i] else 'X' for i in range(len(expect))]))
