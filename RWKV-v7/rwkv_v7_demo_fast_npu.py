...
########################################################################################################
import sys
sys.path.append("/workspace/RWKV-LM/RWKV-v7/ascend/build")
import wkv7s
...
args.MODEL_NAME = "/workspace/models/RWKV-x070-World-0.1B-v2.8-20241210-ctx4096"
...
class WKV_7(torch.autograd.Function):
    ...
    def forward(ctx, state, r, w, k, v, a, b):
        with torch.no_grad():
            ...
            wkv7s.forward(1, T, C, H, state, r, w, k, v, a, b, y)
            return y
...
class RWKV_x070(MyModule):
    def __init__(self, args):
        ...
        self.z = torch.load(args.MODEL_NAME + '.pth', map_location='npu')
        z = self.z
    ...
    def forward(self, idx, state, full_output=False):
        if state == None:
            ...
            for i in range(args.n_layer): # state: 0=att_x_prev 1=att_kv 2=ffn_x_prev
                state[i*3+0] = torch.zeros(args.n_embd, dtype=DTYPE, requires_grad=False, device="npu")
                state[i*3+1] = torch.zeros((args.n_embd // args.head_size, args.head_size, args.head_size), dtype=torch.float, requires_grad=False, device="npu")
                state[i*3+2] = torch.zeros(args.n_embd, dtype=DTYPE, requires_grad=False, device="npu")
...
def RWKV_x070_TMix_seq(layer_id: int, H:int, N:int, x, x_prev, v_first, state, x_r, x_w, x_k, x_v, x_a, x_g, w0, w1, w2, a0, a1, a2, v0, v1, v2, g1, g2, k_k, k_a, r_k, R_, K_, V_, O_, ln_w, ln_b):
    ...
    xx = RWKV7_OP_KERNEL(state, r, w, k, v, -kk, kk*a)
    torch.npu.synchronize()
    # print(xx[0][:64])
...
print(f'\nUsing NPU {str(DTYPE).replace("torch.","")}. Loading {args.MODEL_NAME} ...')
