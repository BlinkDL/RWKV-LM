...
class RWKV_RNN(MyModule):
    def __init__(self, args):
        ...
        for k in keys:
            z[k] = z[k].squeeze()
            if k.endswith('att.w0'): z[k] = z[k].float()
...
for i in range(args.n_layer): # state: 0=att_x_prev 1=att_kv 2=ffn_x_prev
    zero_state[i*3+0] = torch.zeros(args.n_embd, dtype=torch.bfloat16, requires_grad=False, device="cuda")
    ...
    zero_state[i*3+2] = torch.zeros(args.n_embd, dtype=torch.bfloat16, requires_grad=False, device="cuda")
