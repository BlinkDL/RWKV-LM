...
class RWKV_x070(MyModule):
    def __init__(self, args):
        ...
        keys = list(z.keys())
        for k in keys:
            if 'key.weight' in k or 'value.weight' in k or 'receptance.weight' in k or 'output.weight' in k or 'head.weight' in k or 'qq.weight' in k or 'kk.weight' in k  or 'kkup.weight' in k or 'vv.weight' in k  or 'vvup.weight' in k:
                z[k] = z[k].t()
    ...
    def forward(self, idx, state, full_output=False):
        if state == None:
            ...
            state[args.n_layer*3+0] = torch.empty((0), dtype=torch.int, requires_grad=False, device="cuda")
    ...
    def forward_seq(self, idx:List[int], state:List[torch.Tensor], full_output:bool=False):
        with torch.no_grad():
            ...
            for i in range(self.n_layer):
                ...
                q = F.layer_norm(x @ z[qkv+'qq.weight'], (256,), weight=z[qkv+'lnq.weight'], bias=z[qkv+'lnq.bias'])
                
                kk = x @ z[qkv+'kk.weight']
                state[self.n_layer*3+1+i*2] = torch.cat((state[self.n_layer*3+1+i*2], kk), dim=0)
                k = (state[self.n_layer*3+1+i*2] @ z[qkv+'kkup.weight']) * (z[qkv+'ennk.weight'][ctx])
                k = F.layer_norm(k, (256,), weight=z[qkv+'lnk.weight'], bias=z[qkv+'lnk.bias'])

                vv = x @ z[qkv+'vv.weight']
                state[self.n_layer*3+1+i*2+1] = torch.cat((state[self.n_layer*3+1+i*2+1], vv), dim=0)
                v = torch.tanh(state[self.n_layer*3+1+i*2+1] @ z[qkv+'vvup.weight']) * (z[qkv+'emm.weight'][ctx])
