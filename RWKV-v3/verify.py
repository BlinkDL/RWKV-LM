...
model_gpt = RWKV_GPT(
    model_name, RUN_DEVICE, model_type, tokenizer.vocab_size, n_layer, n_embd, ctx_len
).cuda()
...
ctx = [
    ctx
] * 4  # increase batch size (to make it work with B_GROUP_FORWARD & B_GROUP_BACKWARD)
out = (
    model_train.forward(torch.tensor(ctx).cuda())[0][0][:src_len].detach().cpu().numpy()
)
print(out, "\n")
